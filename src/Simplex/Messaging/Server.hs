  ...
  where
    ...
    processCommand clntVersion (q_, (corrId, entId, cmd)) = case cmd of
      ...
      Cmd SLinkClient command -> Just <$> case command of
        LKEY k -> withQueue $ \q qr -> checkMode QMMessaging qr $ secureQueue_ q k $>> getQueueData_ q qr
        LGET -> withQueue $ \q qr -> checkMode QMContact qr $ getQueueData_ q qr
      ...
      Cmd SRecipient command ->
        Just <$> case command of
          ...
          LSET lnkId d -> undefined
          LDEL -> withQueue $ \q qr -> liftIO $
            (corrId,entId,) . either ERR (LNK $ senderId qr)
              getRcvQueueData (queueStore ms) q entId
      ...
      where
        ...
        getQueueData_ :: StoreQueue s -> QueueRec -> M (Either ErrorType BrokerMsg)
        getQueueData_ q qr = liftIO $ LNK (senderId qr) <$$> getQueueLinkData (queueStore ms) q entId
